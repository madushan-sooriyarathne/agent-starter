...
A Claude Code plugin marketplace shipping one plugin — `setup-agents` — that scaffolds a tailored agent workspace (agents, skills, hooks, rules) into any project. Targets **Claude Code** (`.claude/`) and **Google Antigravity** (flat `.agents/`).
...
`setup-agents` is also installable through the `agy` CLI — no separate build
or manifest needed, `agy` reads the `plugins/` layout directly:

```bash
git clone https://github.com/madushan-sooriyarathne/agent-starter
agy plugin install ./agent-starter                    # bulk-detects plugins/ (setup-agents only)
agy plugin install ./agent-starter/plugins/setup-agents # or target it directly
```

`agy plugin install` on a directory containing a `plugins/` folder auto-detects
it as a bulk marketplace and installs every plugin found inside — here, just
`setup-agents`. There's no `agy`-native equivalent of `claude plugin marketplace
add <owner>/<repo>` yet (`agy plugin link` requires a marketplace that's already
registered, and no `agy plugin marketplace add`-style command exists) — cloning
locally and pointing `agy plugin install` at the checkout is the supported path
today. Run `agy plugin list` afterward, then reload Antigravity or start a new
session so `/setup-agents` is picked up.
...
`plugins/setup-agents/` contains a `.claude-plugin/plugin.json`, the
`template/` shipped to user projects, and a real copy of `skills/setup-agents/`
under `plugins/setup-agents/skills/` (regenerated by
`scripts/materialize-agy-skills.sh`). Top-level `agents/`/`rules/`/`hooks/`/`skills/`
are the source of truth — they reach user projects via `/setup-agents`
scaffolding, not as standalone plugins.
