...
if [ ! -d "$TARGET" ]; then
  say "${YELLOW}Path is not a directory:${RESET} $TARGET"
  exit 1
fi
...
case "$REPLY_LINE" in
  ...
  *)
    WANT_CLAUDE=1
    WANT_AG=0
    say "  ${DIM}Unrecognized — defaulting to Claude Code.${RESET}"
    ;;
esac
...
{ [ -f "$TARGET/biome.json" ] || [ -f "$TARGET/biome.jsonc" ]; } && {
  HAS_BIOME=1
  HAS_LINTER=1
}
...
fe=""
[ "$HAS_NEXT" = "1" ] && fe="Next.js"
[ "$HAS_REACT" = "1" ] && fe="${fe:+$fe + }React"
[ "$HAS_TAILWIND" = "1" ] && fe="${fe:+$fe + }Tailwind"
{ [ -z "$fe" ] && [ "$HAS_FRONTEND" = "1" ]; } && fe="(frontend dirs)"
be=""
[ "$HAS_HONO" = "1" ] && be="Hono"
[ "$HAS_BUN" = "1" ] && be="${be:+$be + }Bun"
{ [ -z "$be" ] && [ "$HAS_BACKEND" = "1" ]; } && be="(backend dirs)"
db=""
[ "$HAS_DRIZZLE" = "1" ] && db="Drizzle"
{ [ -z "$db" ] && [ "$HAS_DB" = "1" ]; } && db="(migrations/ORM)"
au=""
[ "$HAS_AUTH" = "1" ] && au="BetterAuth/auth"
ts=""
[ "$HAS_TESTS" = "1" ] && ts="present"
fmt=""
[ "$HAS_BIOME" = "1" ] && fmt="Biome"
{ [ -z "$fmt" ] && [ "$HAS_LINTER" = "1" ]; } && fmt="ESLint/Prettier"
tc=""
[ "$HAS_TS" = "1" ] && tc="tsc"
pm=""
[ "$HAS_BUN" = "1" ] && pm="bun"
[ "$HAS_MONOREPO" = "1" ] && pm="${pm:+$pm }(monorepo)"
[ "$HAS_PKG" = "1" ] && [ -z "$pm" ] && pm="npm/node"
[ "$HAS_GO" = "1" ] && pm="go modules"
[ "$HAS_PY" = "1" ] && pm="${pm:-pip/uv}"
[ "$HAS_RUST" = "1" ] && pm="cargo"
gb="$(git -C "$TARGET" symbolic-ref --short HEAD 2>/dev/null || printf '')"
gh_s=""
command -v gh >/dev/null 2>&1 && gh_s="gh ✓"
[ "$HAS_GH_REMOTE" = "1" ] && gh_s="${gh_s:+$gh_s, }github remote"
git_s="${gb:+$gb}${gh_s:+${gb:+, }$gh_s}"
ci=""
[ "$HAS_CI" = "1" ] && ci="detected"
...
select_all() {
  ...
  if [ -f "$TARGET/CLAUDE.md" ]; then
    ...
    WANT_CLAUDEMD=1
    say "  Copy CLAUDE.md template ${DIM}(auto: $TIER tier)${RESET}"
  else
  ...
  while true; do
    ...
    case "$REPLY_LINE" in
      ...
      c | C | cancel | CANCEL | n | N | no | NO)
        ...
        ;;
      *)
        say "  Unrecognized, treating as approve."
        break
        ;;
    esac
  done
...
if [ "$WANT_AG" = "1" ]; then
    ...
    for r in "${sel_rules[@]}"; do
      if copy_managed "$SCRIPT_DIR/rules/$r.md" "$AG_ROOT/rules/$r.md" "rule: $r"; then
        ...
        if [ -n "$PRIMARY_SRC_DIR" ] && grep -q '^paths:' "$AG_ROOT/rules/$r.md" 2>/dev/null; then
          sed -i.bak "/^paths:/,/^---/ s#\"src/#\"$PRIMARY_SRC_DIR/#g" "$AG_ROOT/rules/$r.md"
          rm -f "$AG_ROOT/rules/$r.md.bak"
        fi
      fi
    done
  fi
  ...
  if [ ${#sel_agents[@]} -gt 0 ]; then
    for a in "${sel_agents[@]}"; do
      ...
      [ -f "$src" ] || {
        skip "agent: $a (source missing)"
        continue
      }
      dest_dir="$AG_ROOT/skills/$a"
      dest="$dest_dir/SKILL.md"
      if [ -e "$dest" ]; then
        skip "skill: $a (exists, kept)"
        continue
      fi
      mkdir -p "$dest_dir"
  ...
  if [ -z "$RUNNER" ]; then
    ...
    for s_name in "${sel_skills[@]}"; do
      ...
      for nm in "${skills_names[@]}"; do
        if [ "$nm" = "$s_name" ]; then
          repo="${skills_repos[$idx]}"
          skn="${skills_skillnames[$idx]}"
          break
        fi
        idx=$((idx + 1))
      done
    ...
    sel_skills=()
    [ ${#installed_skills[@]} -gt 0 ] && sel_skills=("${installed_skills[@]}")
  fi
fi
...
load via the ponytail skill."
      ...
      graphify)
        ...
        if command -v uv >/dev/null 2>&1; then
          GPM="uv tool install graphifyy"
        elif command -v pipx >/dev/null 2>&1; then
          GPM="pipx install graphifyy"
        elif command -v pip >/dev/null 2>&1; then
          GPM="pip install graphifyy"
        fi
        if [ -z "$GPM" ]; then
...
plats=""
[ "$WANT_CLAUDE" = "1" ] && plats="Claude Code"
[ "$WANT_AG" = "1" ] && plats="${plats:+$plats + }Antigravity"
head "Done"
say "  Target:             $TARGET"
